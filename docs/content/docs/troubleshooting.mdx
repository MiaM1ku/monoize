...
Charge = base price × group `billing_ratio` × Channel model `multiplier`. Confirm the model has an enabled price row on the **Models** page. Check the group's billing ratio in **Models → Group Pricing**. Check the multiplier on the Channel model entry. A multiplier of `0` makes the Channel free.
