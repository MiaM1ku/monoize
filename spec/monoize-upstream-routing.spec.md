...
RTA-8a. RTA-8 has one structured-error exception. If the final failed attempt has `upstream_code = "thinking_signature_invalid"`, Monoize MUST return `error.code = "thinking_signature_invalid"` and MUST use the final attempt error as the downstream message without an `All ... upstream attempt(s) failed` wrapper. If the final `upstream_status` is a valid HTTP `4xx` status, Monoize MUST return that status. Otherwise Monoize MUST return HTTP `400`. The request log `error_code`, `error_message`, and `error_http_status` MUST equal the downstream values. Monoize MUST apply this rule only after all eligible Channels and Providers are exhausted; it MUST NOT disable fail-forward.
